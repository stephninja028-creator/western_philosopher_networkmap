...
import { T, SITE, COMMON, BLOG, PHIL_DIR, EPOCH as EPOCH_T, SCHOOL as SCHOOL_T, SECTIONS, API } from "./src/i18n/uiStrings";
import { chatSystem } from "./src/ai/prompts/chat";
import { debateArena, debateMultilateral } from "./src/ai/prompts/debate";
...
${JSON.stringify({ details, lifeAndTimes, worldviewSummary, quote, concepts, comparisons }, null, 2)}`,
  ...
  app.post("/api/debate-arena", debateLimiter, async (req, res) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: debateArena(p1, p2, topic, schoolTranslations, isEn),
        config: {
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
  ...
  app.post("/api/debate-multilateral", debateLimiter, async (req, res) => {
    ...
    try {
      ...
      const systemInstruction = debateMultilateral(philosophers, topic, isEn, schoolTranslations);
